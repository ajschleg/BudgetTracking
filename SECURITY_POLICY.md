...
- **Transactions live on the user's own server** (`server/plaid.db` `transactions` table) as the source of truth since the 2026-06 server-hub migration. They move only between the user's devices and that server, over the user's tailnet (or localhost), gated by `X-App-Token`. They MUST NEVER transit any third-party service. Server logs MUST NEVER contain transaction descriptions, merchants, or amounts (§8) — counts and UUIDs only. At rest they rely on file ACL `0600` + FileVault, the same accepted posture as the rest of plaid.db; the nightly backups in `server/backups/` MUST keep the same permissions and stay gitignored.
- **Every other record type also lives on the user's own server** since Phase 3 (2026-06): budgets, categories, learned rules, monthly snapshots, bank profiles, and imported-file metadata replicate via the opaque-payload `server_records` store. CloudKit and LAN sync are REMOVED — exactly one sync authority exists, and no user data transits Apple or any other third party. PlaidAccount metadata is never peered; each device reads `GET /api/accounts` from the server directly.
...
- 2026-06-10: Server-hub migration — §10 updated: the server's `transactions` table is now the source of truth; devices pull `/api/transactions/changes` and push edits through the token-gated API; transactions excluded from CloudKit/LAN sync when the client ships. Justification: a single shared Plaid cursor split history across devices; the prior "transactions only via the user's iCloud/LAN" rule becomes "transactions never leave the user's own machines" (the server is the user's Mac mini, tailnet-only). §8 logging rules unchanged and now explicitly cover the ingest path; nightly plaid.db backups added (14-day retention, same file posture).
- 2026-06-11: Phase 3 — ALL remaining synced record types (categories, rules, snapshots, bank profiles, imported-file metadata) moved to the server's `server_records` store (opaque JSON payloads; counts-only logging). CloudKit and LAN sync deleted entirely, with their entitlements (iCloud container, aps-environment, Bonjour); the LAN PII-stripping rules above became moot and were removed. iOS is a pure server client (direct Plaid Link via /api/link/*). Database export/import UI removed — recovery is server backups + the Settings "Re-sync from server" action.
